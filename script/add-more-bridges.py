...
def main():
    ...
    bridge = mc.add_bridge().name('DHCPv6BRIDGE NO2')\
                            .tenant_id(my_laptop)\
                            .create()
    bridge.add_exterior_port().create()
    bridge.add_exterior_port().create()
    bridge.add_tag().tag("tomohiko_tag1").create()
    bridge.add_tag().tag("tomohiko_tag3").create()

    bridge = mc.add_bridge().name('DHCPv6BRIDGE NO3')\
                            .tenant_id(my_laptop)\
                            .create()
    bridge.add_exterior_port().create()
    bridge.add_exterior_port().create()
